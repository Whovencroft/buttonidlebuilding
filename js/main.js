(() => {
  ...
  function defaultState() {
    return normalizeHostedState({
      ...
      scenes: {
        ...
        marble: {
          unlocked: false,
          currentLevelId: 'training_ground',
          bestTimes: {},
          clearedLevels: [],
          rewardClaims: {},
          unlockedFlags: []
  ...
  function normalizeHostedState(data) {
    ...
    if (typeof data.scenes.marble.currentLevelId !== 'string' || !data.scenes.marble.currentLevelId) {
      data.scenes.marble.currentLevelId = 'training_ground';
    }
  ...
  function applyMarbleReward(result) {
    ...
    // Check if all 6 levels are now beaten - show secret tunnel hint (only once)
    if (result.levelId === 'level_6' && !marbleSlice.rewardClaims['secret_tunnel'] && window.MarbleLevels && window.MarbleLevels.isSecretRevealed) {
  ...
  function beginEndingTransitionToMarble() {
    ...
    state.scenes.marble.unlocked = true;
    state.scenes.marble.currentLevelId = state.scenes.marble.currentLevelId || 'training_ground';
